...
{
    public class TextCommandManager: CommandTypeManager<NpgsqlParameter, NpgsqlConnection>
    {
        public TextCommandManager(NpgsqlConnection pgsqlConnection) : base(pgsqlConnection) { }
        ...
        public override async Task ExecuteNoneResultsAsync(string commandText, params NpgsqlParameter[] sqlParameters)
        {
            try
            {
                ...
                using (NpgsqlCommand sqlCommand = new NpgsqlCommand(commandText, Connection) { CommandType = CommandType.Text })
                {
